...
First make sure your local ecosystem is running and the SCM dogu and also the user management dogu are both installed.
Now check the `cypress.json` in the `integrationTests` directory. The configuration should match to your instance, like `baseUrl` and the admin settings.
If both dogus are running, navigate your terminal into the `integrationTests` directory and start cypress.

You can run cypress headless like `yarn cypress run` or with the cypress browser like `yarn cypress open`.
